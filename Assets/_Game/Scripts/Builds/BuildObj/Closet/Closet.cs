...
public class Closet : ClosetBase, ILock
{
                ...
                if (isPushEvent)
                {
                    switch (nameObject_This)
                    {
                        case NameObject_This.LionCloset:
                            EnventManager.TriggerEvent(EventName.LionCloset_OnComplete.ToString());
                            break;
                ...
                if (isPushEvent)
                {
                    switch (nameObject_This)
                    {
                        case NameObject_This.CrocCloset:
                            EnventManager.TriggerEvent(EventName.CrocCloset_OnComplete.ToString());
                            break;
                ...
                if (isPushEvent)
                {
                    switch (nameObject_This)
                    {
                        case NameObject_This.EleCloset:
                            EnventManager.TriggerEvent(EventName.EleCloset_OnComplete.ToString());
                            break;
                ...
                if (isPushEvent)
                {
                    switch (nameObject_This)
                    {
                        case NameObject_This.ZebraCloset:
                            EnventManager.TriggerEvent(EventName.ZebraCloset_OnComplete.ToString());
                            break;
    ...
    void Update()
    {
        if (!IsLock)
        {
            ...
            uI_InfoBuild.LoadTextProcess(listOutfits.Count.ToString() + "/" + listOutfitPos.Count.ToString());
        }
        else
        {
